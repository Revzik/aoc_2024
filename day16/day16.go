...
func debugPrint(graph *g.Graph[int], board *s.Board) {
	matrix := make([][]int, board.Height())
	for y, row := range board.Plane {
		matrix[y] = make([]int, board.Width())
		for x := range row {
			matrix[y][x] = -1
		}
	}
	...
	for nCode := range graph.Nodes {
		n := gc.IntToNode(nCode, board)
		matrix[n.Vertex.Y][n.Vertex.X] = nCode

		if nCode > largestCode {
			largestCode = nCode
		}
	}
	...
	for _, row := range matrix {
		...
		for _, item := range row {
			if item == -1 {
				rowStr += strings.Repeat("#", charWidth)
			} else {
				item -= item % 4
				rowStr += fmt.Sprintf("%*d", charWidth, item)
			}
			rowStr += " "
		}
